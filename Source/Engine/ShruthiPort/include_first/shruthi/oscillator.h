...
class Oscillator {
 public:
  ...
  inline void set_random(avrlib::Random* random) {
    random_ = random;
  }

  inline void set_user_wavetable(uint8_t* wavetable) {
    user_wavetable_ = wavetable;
  }

  inline void Reset() {
    data_.no.rng_reset_value = (random_ != nullptr ? random_->GetByte() : 0x21) + 1;
  }
  ...
  inline void Render(
    ...
    uint8_t index = shape_ > WAVEFORM_VOWEL ? WAVEFORM_WAVETABLE_1 : shape_;
    switch (index) {
      case WAVEFORM_NONE: RenderSilence(buffer); break;
      case WAVEFORM_SAW: RenderSimpleWavetable(buffer); break;
      case WAVEFORM_TRIANGLE: RenderSimpleWavetable(buffer); break;
      case WAVEFORM_CZ_SAW: RenderCzSaw(buffer); break;
      case WAVEFORM_CZ_RESO: RenderCzReso(buffer); break;
      case WAVEFORM_CZ_TRIANGLE: RenderCzReso(buffer); break;
      case WAVEFORM_CZ_PULSE: RenderCzPulseReso(buffer); break;
      case WAVEFORM_CZ_SYNC: RenderCzReso(buffer); break;
      case WAVEFORM_QUAD_SAW_PAD: RenderQuadSawPad(buffer); break;
      case WAVEFORM_FM: RenderFm(buffer); break;
      ...
      case WAVEFORM_WAVETABLE_8:
        RenderInterpolatedWavetable(buffer);
        break;
      case WAVEFORM_WAVETABLE_USER: RenderSweepingWavetableRam(buffer); break;
      case WAVEFORM_8BITLAND: Render8BitLand(buffer); break;
      case WAVEFORM_CRUSHED_SINE: RenderCrushedSine(buffer); break;
      case WAVEFORM_DIRTY_PWM: RenderDirtyPwm(buffer); break;
      case WAVEFORM_FILTERED_NOISE: RenderFilteredNoise(buffer); break;
      case WAVEFORM_VOWEL: RenderVowel(buffer); break;
      default:
        if (shape_ >= WAVEFORM_WAVETABLE_9) {
          RenderInterpolatedWavetable(buffer);
        } else {
 ...
 private:
  ...
  uint8_t* sync_input_ = nullptr;
  uint8_t* sync_output_ = nullptr;
