...
static const Parameter dummy_parameter = {
  0xff, UNIT_UINT8, 0, 255, { 0, 0 }, 0, 0,
};

uint8_t ParameterManager::for_cc(uint8_t cc) {
  cc &= 0x7f;
  for (uint8_t i = 0; i < kNumParameters; ++i) {
    const Parameter& p = parameter(i);
    if ((p.midi_cc[0] != 0 && p.midi_cc[0] == cc) ||
        (p.midi_cc[1] != 0 && p.midi_cc[1] == cc)) {
      return i;
    }
  }
  return 0xff;
...
uint8_t ParameterManager::at_offset(uint8_t offset) {
  if (offset >= 160) {
    return 0xff;
  }
  ...
  for (uint8_t i = 0; i < kModulationMatrixSize; ++i) {
    if (offset == PRM_MOD_SOURCE + i * 3) {
      return 33;
    }
    if (offset == PRM_MOD_SOURCE + i * 3 + 1) {
      return 34;
    }
    if (offset == PRM_MOD_SOURCE + i * 3 + 2) {
      return 35;
    }
  }

  for (uint8_t i = 0; i < 2; ++i) {
    if (offset == PRM_OP_OP1 + i * 3) {
      return 59;
    }
    if (offset == PRM_OP_OP1 + i * 3 + 1) {
      return 60;
    }
    if (offset == PRM_OP_OP1 + i * 3 + 2) {
      return 61;
    }
  }
...
const Parameter& ParameterManager::parameter(
    uint8_t index) {
  if (index >= kNumParameters || index == 0xfe) {
