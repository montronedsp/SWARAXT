...
void Part::Init(HostAudioRing& audio_out,
  ...
  random_ = &random;
  storage_->Init(this);
  midi_dispatcher_->Init(this, storage_);
  ResetPatch();
  ...
  voice_.Init(this, &audio_out, &random);
  dirty_ = false;
...
static const prog_char init_system_settings[] PROGMEM = {
    ...
    8, 0, 0, 0,
    0, 0, 0,
    // Upstream stops here: on AVR sizeof(SystemSettings) is 15 because uint16_t
    // needs no alignment. On the host the struct is padded to 16, so the
    // resource copy would read one byte past this table.
    0,
};

static_assert(sizeof(init_system_settings) >= sizeof(SystemSettings),
              "host SystemSettings padding requires a padded init table");
  ...
  switch (controller) {
    ...
    case midi::kDataDecrement:
      ...
      if (nrpn_parameter_number_ != 0xff) {
        uint8_t parameter_index = ParameterManager::at_offset(
    ...
    default:
      {
        uint8_t index = ParameterManager::for_cc(controller);
        if (index == 0xff) {
...
void Part::Clock(bool internal) {
  if (running()) {
    ...
    if (!arp_seq_gate_length_counter_) {
      if (generated_notes_.size()) {
...
void Part::AlignExternalClock(uint64_t ticksBeforeNextClock) {
  Start(false);
  const uint8_t duration = step_duration();
  if (ticksBeforeNextClock == 0) {
    arp_seq_step_ = 0xff;
    arp_seq_prescaler_ = 0;
  } else {
    arp_seq_step_ = static_cast<uint8_t>(((ticksBeforeNextClock - 1) / duration) & 0x0f);
    arp_seq_prescaler_ = static_cast<uint8_t>(ticksBeforeNextClock % duration);
  }
  lfo_step_[0] = static_cast<uint16_t>(ticksBeforeNextClock % 1536);
  lfo_step_[1] = lfo_step_[0];
  arp_seq_gate_length_counter_ = 0;
}

void Part::SetHostLfoSync(uint8_t index, bool enabled, uint16_t phaseIncrement) {
  if (index >= kNumLfos) {
    return;
  }
  host_lfo_sync_[index] = enabled;
  host_lfo_increment_[index] = phaseIncrement;
  if (enabled) {
    lfo_[index].set_target_increment(phaseIncrement);
  } else {
    UpdateLfoRate(index);
  }
}

void Part::SetGatePercent(uint8_t percent) {
  gate_percent_ = percent < 5 ? 5 : (percent > 100 ? 100 : percent);
}
/* static */
void Part::Start(bool internal) {
  if (internal) {
    midi_dispatcher_->OnStart();
  }
  ...
  if (sequencer_settings_.mode() == SEQUENCER_MODE_ARP &&
    ...
    if (num_notes == 1 && sequencer_settings_.arp_range == 1) {
      ...
      if (sequencer_settings_.arp_direction == ARPEGGIO_DIRECTION_RANDOM) {
        uint8_t random_byte = random_ != nullptr ? random_->state_msb() : 0;
    ...
    arp_seq_gate_length_counter_ =
        static_cast<uint16_t>((step_duration() * gate_percent_ + 99) / 100);
  }
...
void Part::SetScaledParameter(
    ...
    bool user_initiated) {
  dirty_ = true;
  const Parameter& parameter = ParameterManager::parameter(parameter_index);
  SetParameter(
      parameter_index,
      parameter.offset,
...
void Part::SetParameter(
    ...
    uint8_t value,
    bool user_initiated) {
  uint8_t* byte = parameter_byte(offset);
  if (byte == NULL || *byte == value) {
    return;
  }
  *byte = value;
  ...
  if (user_initiated) {
    midi_dispatcher_->OnEdit(parameter_index, offset, value);
  }
}

uint8_t Part::GetParameter(uint8_t offset) const {
  const uint8_t* byte = parameter_byte(offset);
  return byte != NULL ? *byte : 0;
}

uint8_t* Part::parameter_byte(uint8_t offset) {
  if (offset < sizeof(Patch)) {
    return reinterpret_cast<uint8_t*>(&patch_) + offset;
  }

  offset = static_cast<uint8_t>(offset - sizeof(Patch));
  if (offset < sizeof(SequencerSettings)) {
    return reinterpret_cast<uint8_t*>(&sequencer_settings_) + offset;
  }

  offset = static_cast<uint8_t>(offset - sizeof(SequencerSettings));
  if (offset < sizeof(SystemSettings)) {
    return reinterpret_cast<uint8_t*>(&system_settings_) + offset;
  }

  return NULL;
}

const uint8_t* Part::parameter_byte(uint8_t offset) const {
  if (offset < sizeof(Patch)) {
    return reinterpret_cast<const uint8_t*>(&patch_) + offset;
  }

  offset = static_cast<uint8_t>(offset - sizeof(Patch));
  if (offset < sizeof(SequencerSettings)) {
    return reinterpret_cast<const uint8_t*>(&sequencer_settings_) + offset;
  }

  offset = static_cast<uint8_t>(offset - sizeof(SequencerSettings));
  if (offset < sizeof(SystemSettings)) {
    return reinterpret_cast<const uint8_t*>(&system_settings_) + offset;
  }
...
/* static */
void Part::ProcessBlockInternal(bool render_audio) {
  ...
  voice_.set_modulation_source(MOD_SRC_NOISE, random_ != nullptr ? random_->state_msb() : 0);
  ...
  if (render_audio) {
    ...
    voice_.ProcessControlBlock();
  }
}

void Part::ProcessBlock() {
  ProcessBlockInternal(true);
}

void Part::ProcessControlBlock() {
  ProcessBlockInternal(false);
}
...
void Part::ReleaseLatchedNotes() {
  for (uint8_t i = 1; i <= pressed_keys_.max_size(); ++i) {
    NoteEntry* e = pressed_keys_.mutable_note(i);
    if (e->velocity & 0x80) {
      NoteOff(midi_dispatcher_->channel(), e->note);
    }
  }
}
...
uint16_t Part::Tune(uint8_t note) {
  if (system_settings_.raga) {
    int16_t pitch_shift = ResourcesManager::Lookup<int16_t, uint8_t>(
        ResourceId(LUT_RES_SCALE_JUST + system_settings_.raga - 1),
        note % 12);
...
void Part::InternalNoteOn(uint8_t note, uint8_t velocity) {
  midi_dispatcher_->OnInternalNoteOn(note, velocity);
  ...
  if (system_settings_.midi_out_mode >= MIDI_OUT_1_0) {
    ...
    if (poly_allocator_.NoteOn(note) == 0) {
      ...
      midi_dispatcher_->ForwardNoteOn(midi_dispatcher_->channel(), note, velocity);
    }
  } else {
...
void Part::InternalNoteOff(uint8_t note) {
  ...
  if (system_settings_.midi_out_mode >= MIDI_OUT_1_0) {
    ...
    if (poly_allocator_.NoteOff(note) == 0) {
      ...
      midi_dispatcher_->ForwardNoteOff(midi_dispatcher_->channel(), note);
    }
  } else {
    const NoteEntry& before = mono_allocator_.most_recent_note();
