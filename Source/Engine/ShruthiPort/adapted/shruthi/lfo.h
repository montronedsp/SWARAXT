...
class Lfo {
 public:
  ...
  void Init(avrlib::Random* random) {
    random_ = random;
  }
  ...
  uint8_t Render(const SequencerSettings& sequence) {
    ...
    switch (shape_) {
      ...
      case LFO_WAVEFORM_S_H:
        if (cycle_complete_) {
          value_ = random_ != NULL ? random_->GetByte() : value_;
        }
        value = value_;
        break;
 ...
 private:
  ...
  uint16_t previous_phase_ = 0;
  uint16_t intensity_increment_ = 0;
  uint16_t target_increment_ = 0;
