...
void Voice::Init(Part* part, HostAudioRing* audio_out, avrlib::Random* random) {
  ...
  random_ = random;
  osc_1_.set_random(random_);
  osc_2_.set_random(random_);
  osc_1_.set_user_wavetable(user_wavetable_);
  osc_2_.set_user_wavetable(user_wavetable_);
  pitch_value_ = 60 << 7;
...
void Voice::RefreshEnvelopeRatesFromPatch() {
  for (uint8_t i = 0; i < kNumEnvelopes; ++i) {
    const EnvelopeSettings& settings = part_->patch_.env[i];
    envelope_[i].Update(
        settings.attack,
        settings.decay,
        settings.sustain,
...
void Voice::NoteOn(
  ...
  if (trigger || pitch_value_ == 0) {
    ...
    modulation_sources_[MOD_SRC_RANDOM] = random_ != nullptr ? random_->state_msb() : 0;
    osc_1_.Reset();
    osc_2_.Reset();
  }
...
inline void Voice::LoadSources() {
  ...
  for (uint8_t i = 0; i < 2; ++i) {
    if (!part_->patch_.ops_[i].op) {
      continue;
    }
    uint8_t x = part_->patch_.ops_[i].operands[0];
    uint8_t y = part_->patch_.ops_[i].operands[1];
    x = modulation_sources_[x];
    y = modulation_sources_[y];
    uint8_t op = part_->patch_.ops_[i].op;
    if (op <= OP_CV_LE) {
      if (x > y) {
        ops[3] = x;  ops[6] = 255;
        ops[4] = y;  ops[7] = 0;
  ...
  dst_[MOD_DST_FILTER_CUTOFF] = U8U8Mul(
      part_->patch_.filter_cutoff, 128);

  dst_[MOD_DST_PWM_1] = U8U8Mul(part_->patch_.osc[0].parameter, 128);
  dst_[MOD_DST_PWM_2] = U8U8Mul(part_->patch_.osc[1].parameter, 128);
  ...
  dst_[MOD_DST_FILTER_RESONANCE] = part_->patch_.filter_resonance << 8;
  dst_[MOD_DST_MIX_BALANCE] = part_->patch_.mix_balance << 8;
  dst_[MOD_DST_MIX_NOISE] = part_->patch_.mix_noise << 8;
  dst_[MOD_DST_MIX_SUB_OSC] = part_->patch_.mix_sub_osc << 8;

  dst_[MOD_DST_ATTACK] = 8192;
  int16_t* envelope_parameters = &dst_[MOD_DST_ATTACK_1];
  for (uint8_t i = 0; i < kNumEnvelopes; ++i) {
    *envelope_parameters++ = U8U8Mul(part_->patch_.env[i].attack, 128);
    *envelope_parameters++ = U8U8Mul(part_->patch_.env[i].decay, 128);
    *envelope_parameters++ = U8U8Mul(part_->patch_.env[i].sustain, 128);
    *envelope_parameters++ = U8U8Mul(part_->patch_.env[i].release, 128);
  }
  dst_[MOD_DST_CV_1] = 0;
  dst_[MOD_DST_CV_2] = 0;
  if (part_->system_settings_.expansion_filter_board >= FILTER_BOARD_SSM) {
    dst_[MOD_DST_CV_1] = U8U8Mul(
        part_->patch_.filter_cutoff_2, 128);
    dst_[MOD_DST_CV_2] = part_->patch_.filter_resonance_2 << 8;
  }
}
...
inline void Voice::ProcessModulationMatrix() {
  ...
  for (uint8_t i = 0; i < kModulationMatrixSize; ++i) {
    int8_t amount = part_->patch_.modulation_matrix.modulation[i].amount;
    if (!amount) {
      continue;
    }
    ...
    uint8_t source = part_->patch_.modulation_matrix.modulation[i].source;
    uint8_t destination =
        part_->patch_.modulation_matrix.modulation[i].destination;
...
inline void Voice::UpdateDestinations() {
  ...
  if (part_->patch_.osc[0].option != OP_DUO) {
    if (part_->system_settings().expansion_filter_board == FILTER_BOARD_PVK) {
      cutoff = S16ClipU14(cutoff + pitch_value_ - 8192 + (16 << 7));
    } else {
      cutoff = S16ClipU14(cutoff + pitch_value_ - 8192);
    }
  }
  cutoff = S16ClipU14(cutoff + S8U8Mul(part_->patch_.filter_env,
      modulation_sources_[MOD_SRC_ENV_1]));
  cutoff = S16ClipU14(cutoff + S8S8Mul(part_->patch_.filter_lfo,
  ...
  if (part_->system_settings_.expansion_filter_board == FILTER_BOARD_SVF &&
      part_->patch_.filter_1_mode_ >= FILTER_MODE_LP_COUPLED) {
  ...
  osc_1_.set_parameter(U15ShiftRight7(dst_[MOD_DST_PWM_1]));
  osc_1_.set_secondary_parameter(part_->patch_.osc[0].range + 24);
  osc_2_.set_parameter(U15ShiftRight7(dst_[MOD_DST_PWM_2]));
  osc_2_.set_secondary_parameter(part_->patch_.osc[1].range + 24);

  int8_t attack_mod = (U15ShiftRight7(dst_[MOD_DST_ATTACK]) - 64) * 2;
  int16_t* envelope_parameters = &dst_[MOD_DST_ATTACK_1];
...
inline void Voice::RenderOscillators() {
  ...
  base_pitch += part_->system_settings_.master_tuning;
  ...
  for (uint8_t i = 0; i < kNumOscillators; ++i) {
    ...
    // the duophonic mode.
    if (part_->patch_.osc[0].option == OP_DUO && i == 0 && pitch_bass_note_) {
      pitch -= pitch_value_;
    ...
    int8_t range = 0;
    if (part_->patch_.osc[i].shape != WAVEFORM_FM) {
      range += part_->patch_.osc[i].range;
    }
    range += part_->system_settings_.octave * 12;
    // -24 / +24 semitones by the main octave controller.
    pitch += S8U8Mul(range, 128);
    if (i == 1) {
      // 0 / +1 semitones by the detune option for oscillator 2.
      pitch += part_->patch_.osc[1].option;
    }
    ...
    if (i == 0) {
      ...
      uint8_t shape = part_->patch_.osc[1].shape;
      ...
      osc_2_.Render(
          ...
          midi_note,
          increment,
          part_->patch_.osc[0].option == OP_SYNC ? sync_state_ : no_sync_,
          dummy_sync_state_,
          osc2_buffer_);
    }
  }
...
void Voice::ProcessBlock() {
  ...
  memcpy(debug_osc1_buffer_, buffer_, kAudioBlockSize);
  memcpy(debug_osc2_buffer_, osc2_buffer_, kAudioBlockSize);
  ++debug_process_block_count_;
#endif

  uint8_t op = part_->patch_.osc[0].option;
  ...
  if (part_->patch_.mix_sub_osc_shape < WAVEFORM_SUB_OSC_CLICK) {
    sub_osc_.Render(part_->patch_.mix_sub_osc_shape, buffer_, sub_gain);
  } else {
    if (sub_gain < 128) {
      sub_gain <<= 1;
    }
    transient_generator_.Render(
        part_->patch_.mix_sub_osc_shape, buffer_, sub_gain);
  }
